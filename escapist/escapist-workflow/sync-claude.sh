...
#
# CLAUDE.md는 복사하지 않는다 — 코드 레포가 컨테이너(escapist/) 안에 물리적으로
...
WORKFLOW_DIR="$(cd "$(dirname "$0")" && pwd)"           # .../escapist/escapist-workflow
CONTAINER_DIR="$(cd "$WORKFLOW_DIR/.." && pwd)"          # .../escapist
VP_ROOT="$(cd "$CONTAINER_DIR/.." && pwd)"              # voltron-personal
CODE_REPO="${1:-$CONTAINER_DIR/escapist-code}"
